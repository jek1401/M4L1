...
def hide_img(img_name):
    image = cv2.imread(f'C:\\Users\\user\\Documents\\IT 3 python\\M4L11\\M4L1\\img\\{img_name}')
    blurred_image = cv2.GaussianBlur(image, (15, 15), 0)
    pixelated_image = cv2.resize(blurred_image, (30, 30), interpolation=cv2.INTER_NEAREST)
    pixelated_image = cv2.resize(pixelated_image, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST)
    cv2.imwrite(f'C:\\Users\\user\\Documents\\IT 3 python\\M4L11\\M4L1\\hidden_img\\{img_name}', pixelated_image)

if __name__ == '__main__':
    manager = DatabaseManager(DATABASE)
    manager.create_tables()
    prizes_img = os.listdir('C:\\Users\\user\\Documents\\IT 3 python\\M4L11\\M4L1\\img')
